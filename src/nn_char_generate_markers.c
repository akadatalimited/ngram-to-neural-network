...
static void marker_buf_append(char *buf, size_t buf_size, char ch)
{
...
static void marker_buf_clear(char *buf)
{
    buf[0] = '\0';
}

static int ends_with(const char *buf, const char *suffix)
{
...
static void flush_marker_buf_plain(char *buf)
{
    if (buf[0] != '\0') {
        fputs(buf, stdout);
        marker_buf_clear(buf);
    }
}

static int handle_marker_stream(char *marker_buf, unsigned char ch, int *verse_count)
{
    marker_buf_append(marker_buf, MARKER_BUF, (char)ch);

    if (ends_with(marker_buf, "STARTTOK")) {
        marker_buf_clear(marker_buf);
        return 0;
    }

    if (ends_with(marker_buf, "ENDTOK")) {
        marker_buf_clear(marker_buf);
        putchar('\n');
        (*verse_count)++;
        return 1;
    ...
    if (strlen(marker_buf) > 8) {
        putchar(marker_buf[0]);
        memmove(marker_buf, marker_buf + 1, strlen(marker_buf));
    }
...
{
    fprintf(stderr,
            "usage: %s model.bin seed_text output_len temperature max_verses [mode] [seed]\n"
...
int main(int argc, char *argv[])
{
    ...
    if (max_verses <= 0) {
        fprintf(stderr, "max_verses must be > 0\n");
        return 1;
    }
    ...
    if (argc == 8) {
        rand_seed = (unsigned int)strtoul(argv[7], NULL, 10);
    } else {
    ...
    for (step = 0; step < output_len; step++) {
        ...
        if (handle_marker_stream(marker_buf, next_ch, &verse_count)) {
    ...
    flush_marker_buf_plain(marker_buf);
    putchar('\n');
