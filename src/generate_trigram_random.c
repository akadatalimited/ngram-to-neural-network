    ...
    *used = 0;
    ...
    while (fscanf(fp, "%127s\t%127s\t%127s\t%lu", f, s, t, &count) == 4) {
        if (strcmp(f, first) == 0 && strcmp(s, second) == 0) {
            ...
            snprintf((*candidates)[*used].word, MAX_WORD_LEN, "%s", t);
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4 || argc > 6) {
        fprintf(stderr, "usage: %s trigrams.txt word1 word2 [max_words] [seed]\n", argv[0]);
        return 1;
    }
    ...
    if (strcmp(w1, "starttok") != 0) {
        printf("%s", w1);
        if (strcmp(w2, "endtok") != 0 && strcmp(w2, "starttok") != 0) {
            printf(" %s", w2);
        }
    } else {
        if (strcmp(w2, "endtok") != 0 && strcmp(w2, "starttok") != 0) {
    ...
    for (i = 0; i < max_words; i++) {
        if (!load_candidates(filename, w1, w2, &candidates, &used, &capacity)) {
            free(candidates);
            return 1;
        }
